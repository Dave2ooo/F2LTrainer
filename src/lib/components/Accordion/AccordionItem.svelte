...
<h2 class={`group ${className}`}>
	<button
		...
		class={`flex w-full items-center justify-between border-b border-gray-200 p-5 text-left font-medium text-gray-500 group-first:rounded-t-xl group-first:border-t hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-800 dark:focus:ring-gray-800 ${
	...
	>
		{#if header}
			{@render header()}
			<svg
				class="h-3 w-3 text-gray-800 transition-transform dark:text-white {open
					? 'rotate-180'
					: ''}"
				aria-hidden="true"
