execMain(function () {
		...
		return GiikerCube.waitForAdvs()
			.then(function (mfData) {
				...
				if (dataView && dataView.byteLength >= 6) {
					...
					for (var i = 0; i < 6; i++) {
						mac.push(
							(dataView.getUint8(dataView.byteLength - i - 1) + 0x100).toString(16).slice(1)
						);
					}
	...
	function parseV3Data(value) {
		...
		if (mode == 1) {
			...
			var ts = parseInt(
				value.slice(48, 56) + value.slice(40, 48) + value.slice(32, 40) + value.slice(24, 32),
				2
			);
			...
			var hardwareVersion =
				parseInt(value.slice(80, 84), 2) + '.' + parseInt(value.slice(84, 88), 2);
			var softwareVersion =
				parseInt(value.slice(72, 76), 2) + '.' + parseInt(value.slice(76, 80), 2);
			var devName = '';
			for (var i = 0; i < 5; i++)
	...
	function parseV4Data(value) {
		...
		if (mode == 0x01) {
			...
			var ts = parseInt(
				value.slice(40, 48) + value.slice(32, 40) + value.slice(24, 32) + value.slice(16, 24),
				2
			);
